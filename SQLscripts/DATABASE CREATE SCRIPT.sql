...
CREATE OR REPLACE PROCEDURE prc_new_detail 
	...
	BEGIN
		IF (SELECT VID_NUM FROM VIDEO WHERE VID_NUM = vid_num) IS NOT NULL THEN
			SELECT PRICE_RENTFEE, PRICE_DAILYLATEFEE INTO rentfee, dailylatefee	FROM PRICE JOIN MOVIE JOIN VIDEO WHERE VID_NUM = vid_num;
			INSERT INTO DETAILRENTAL (VID_NUM, DETAIL_FEE, DETAIL_DUEDATE, DETAIL_DAILYLATEFEE, DETAIL_RETURNDATE) VALUES (vid_num, rentfee, ADDDATE(SYSDATE,15), dailylatefee, NULL);
		END IF;
	END;

CREATE OR REPLACE TRIGGER trg_due_date
	BEFORE INSERT ON  DETAILRENTAL
	BEGIN
		UPDATE DETAILRENTAL
		SET DETAILDUEDATE = ADDDATE(SYSDATE,15)
		WHERE RENT_NUM = NEW RENT_NUM AND VID_NUM = NEW VID_NUM;
	END;